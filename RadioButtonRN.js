...
class RadioButtonRN extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        if (this.state.activeIndex === -1 && this.props.initial > 0) {
            ...
            this._initRadio(this.props.data[initialActive], initialActive);
        }
        if (this.props.initial !== prevProps.initial && this.props.isActive) {
            const initialActive = this.props.initial - 1;
	...
	render() {
        ...
        let { boxStyle, style, circleSize, textStyle, data, icon, rightContent = null, activeColor, deactiveColor, boxActiveBgColor, boxDeactiveBgColor, box, textColor, isDisable } = this.props;
                ...
                data.map((item, index) => {
                    return (
                        <TouchableOpacity
                            ...
                            activeOpacity={0.9}
                            onPress={() => {
                                if (!isDisable)this._changeRadio(item, index)
                            }}
                        >
                            <View style={styles.leftProductBox}>
                                <View style={[ icon ? styles.icon : styles.circle, {
                                    borderColor: activeIndex === index ? activeColor : isDisable == false ? deactiveColor : 'transparent',
                                    width: activeIndex === index ? circleSize + 8 : isDisable == false ? circleSize + 8 : 0,
                                    height: activeIndex === index ? circleSize + 8 : isDisable == false ? circleSize + 8 : 0
                                },
                                icon && {
                                    borderColor: activeIndex === index ? 'transparent' : isDisable == false ? deactiveColor : 'transparent'
                                }
                                ]}>
                                    <Animated.View style={{
